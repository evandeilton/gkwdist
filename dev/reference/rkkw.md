...
The kkw quantile function is: \$\$ Q(p) = \left\[ 1 - \left\\ 1 -
\left\[ 1 - (1 - p)^{1/(\delta+1)} \right\]^{1/\lambda}
\right\\^{1/\beta} \right\]^{1/\alpha} \$\$ Random deviates are
generated by evaluating \\Q(p)\\ where \\p\\ is a random variable
following the standard Uniform distribution on (0, 1)
