...
\$\$ for \\0 \< x \< 1\\ and \\\theta = (\alpha, \beta)\\. The
...
(\beta-1)\ln(v_i)\] \$\$ where \\v_i = 1 - x_i^{\alpha}\\. This function
computes and returns the *negative* log-likelihood,
...
`grkw` (gradient, if available), `hskw` (Hessian, if available),
