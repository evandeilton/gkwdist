# gkwdist: Generalized Kumaraswamy Distribution Family ![gkwdist logo](reference/figures/gkwdist.png)
...
and its seven nested sub-models for bounded continuous data on $`(0,1)`$
— proportions, rates, shares, indices. It provides density,
distribution, quantile and random generation functions, and
**analytical** log-likelihood, score and Hessian functions, all written
in C++ via RcppArmadillo.

- Seven nested distributions under one naming rule: prefix + family
  code.
- `ll*`, `gr*` and `hs*` drop straight into
  [`stats::optim`](https://rdrr.io/r/stats/optim.html) — one pass over
  the data, substantially faster than numerical (Richardson)
  differentiation.
- Numerically stable in log space for observations near the boundaries,
  and the densities carry the limiting value at $`x = 0`$ and $`x = 1`$
  as base R does.

## Cheat sheet

**[gkwdist cheat sheet
(PDF)](https://evandeilton.github.io/gkwdist/cheatsheet/gkwdist-cheatsheet.pdf)**
· [view in the
browser](https://evandeilton.github.io/gkwdist/cheatsheet/gkwdist-cheatsheet.html)

Two pages covering the whole package: the nesting tree, all 49 functions
in one matrix, the `d`/`p`/`q`/`r` contract, a gallery of the shapes the
family reaches, the maximum-likelihood recipe, the per-family `par`
ordering, and the nested-model map with likelihood-ratio degrees of
freedom.
...
Every model below is GKw with parameters held fixed. Follow an edge to
fix a parameter and drop to a simpler model.
...
`d*`, `p*`, `q*` and `r*` follow the base R convention. `ll*`, `gr*` and
`hs*` take `(par, data)` and return the **negative** log-likelihood,
score and Hessian, so they minimise directly under
[`optim()`](https://rdrr.io/r/stats/optim.html) and `hs*()` at the
estimate *is* the observed information.

Note the one irregular name: the Beta `d`/`p`/`q`/`r` keep a trailing
underscore so they do not mask
[`stats::dbeta`](https://rdrr.io/r/stats/Beta.html), but the likelihood
trio does not — `llbeta`, not `llbeta_`.
...
Maximum likelihood, with the analytical gradient and the observed
information:
...
returns a named vector already in the order that family’s `ll*` expects
— which differs between families, and is worth checking on the cheat
sheet before writing `par` by hand.

## Learn more

- **[Cheat
  sheet](https://evandeilton.github.io/gkwdist/cheatsheet/gkwdist-cheatsheet.pdf)**
  — the whole package on two pages.
- [`vignette("gkwdist")`](https://evandeilton.github.io/gkwdist/articles/gkwdist.md)
  — a worked introduction: fitting, model selection, profile likelihood,
  confidence regions, diagnostics.
- [`vignette("theory-gkwdist")`](https://evandeilton.github.io/gkwdist/articles/theory-gkwdist.md)
  — the densities, CDFs and quantiles of all seven sub-families with
  proofs, the score and observed information in closed form, and the
  identifiability and boundary conditions behind the asymptotics.
- [Function reference](https://evandeilton.github.io/gkwdist/reference/)
  — every function, with runnable examples.
...
**José Evandeilton Lopes** LEG - Laboratory of Statistics and
Geoinformation PPGMNE - Graduate Program in Numerical Methods in
Engineering Federal University of Paraná (UFPR), Brazil Email:
...
MIT License. See
[LICENSE](https://evandeilton.github.io/gkwdist/LICENSE) file for
details.
