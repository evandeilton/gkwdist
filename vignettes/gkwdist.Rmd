...
- **High performance**: analytical score and Hessian roughly 9× and 38× faster than numerical differentiation
...
for $x \in (0,1)$, with $\alpha, \beta, \gamma, \lambda > 0$ and $\delta \geq 0$, where $B(\cdot, \cdot)$ denotes the beta function.
...
The family does not appear to reach bimodality. Across roughly 330,000 parameter
vectors spanning $(10^{-3}, 300)$ in each of the five parameters, no density with
two interior modes was found: every shape was monotone, unimodal or U-shaped. A
second peak arises only as a divergence at $x = 0$ or $x = 1$, never as a second
interior hump. Data with two separated interior modes calls for a mixture rather
than a larger member of this family.
